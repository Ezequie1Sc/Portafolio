import './Footer.css';
const NAV_LINKS = [
  { label: 'Inicio',      href: '#inicio' },
  { label: 'Sobre mí',    href: '#sobre-mi' },
  { label: 'Proyectos',   href: '#proyectos' },
  { label: 'Habilidades', href: '#habilidades' },
  { label: 'Contacto',    href: '#contacto' },
];

const Footer = () => {
  const year = new Date().getFullYear();

  const datetime = new Date().toLocaleString('es-ES', {
    day:      'numeric',
    month:    'short',
    year:     'numeric',
    hour:     '2-digit',
    minute:   '2-digit',
    hour12:   true,
    timeZone: 'America/Chicago',
  });

  return (
    <footer className="footer">

      {/* Línea superior degradada */}
      <div className="footer-top-line" />

      <div className="footer-container">

        {/* ══════════════════════════════════════════════
            GRID PRINCIPAL
        ══════════════════════════════════════════════ */}
        <div className="footer-content">

          {/* ── COL 1: Marca ── */}
          <div className="footer-brand">
            <div className="footer-logo-wrap">
              <div className="footer-monogram" aria-hidden="true">ES</div>
              <p className="footer-name">Ezequiel Salazar</p>
            </div>

            <p className="footer-tagline">
              Elegancia en diseño, eficiencia en código, impacto en usuarios.
            </p>

            <a
              href="mailto:ezequielsc017@gmail.com"
              className="footer-email-link"
              aria-label="Enviar correo a Ezequiel Salazar"
            >
              <svg width="13" height="13" viewBox="0 0 24 24" fill="none"
                stroke="currentColor" strokeWidth="2"
                strokeLinecap="round" strokeLinejoin="round">
                <rect x="2" y="4" width="20" height="16" rx="2" />
                <path d="m22 7-8.97 5.7a1.94 1.94 0 0 1-2.06 0L2 7" />
              </svg>
              ezequielsc017@gmail.com
            </a>
          </div>

          {/* ── COL 2: Navegación ── */}
          <nav aria-label="Footer navigation">
            <p className="footer-col-label">Navegación</p>
            <ul className="footer-nav-list">
              {NAV_LINKS.map(({ label, href }) => (
                <li key={href}>
                  <a href={href}>{label}</a>
                </li>
              ))}
            </ul>
          </nav>

          {/* ── COL 3: Redes sociales ── */}
          <div className="footer-social">
            <p className="footer-col-label">Redes</p>

            <div className="footer-social-panel">

              {/* LinkedIn */}
              <a
                href="https://www.linkedin.com/in/ezequiel-salazar-194975340/"
                target="_blank"
                rel="noopener noreferrer"
                className="footer-social-item"
                aria-label="LinkedIn de Ezequiel Salazar"
              >
                <span className="footer-social-icon-wrap linkedin-icon">
                  {/* Icono oficial LinkedIn */}
                  <svg width="17" height="17" viewBox="0 0 24 24" fill="currentColor">
                    <path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853
                      0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9
                      1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337
                      7.433a2.062 2.062 0 0 1-2.063-2.065 2.064 2.064 0 1 1 2.063
                      2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0
                      0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24
                      23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z"/>
                  </svg>
                </span>
                <span className="footer-social-text">
                  <span className="footer-social-platform">LinkedIn</span>
                  <span className="footer-social-user">Ezequiel Salazar</span>
                </span>
                <svg className="footer-social-ext" width="13" height="13"
                  viewBox="0 0 24 24" fill="none" stroke="currentColor"
                  strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                  <path d="M7 17L17 7M17 7H7M17 7v10" />
                </svg>
              </a>

              {/* GitHub */}
              <a
                href="https://github.com/Ezequie1Sc"
                target="_blank"
                rel="noopener noreferrer"
                className="footer-social-item"
                aria-label="GitHub de Ezequiel Salazar"
              >
                <span className="footer-social-icon-wrap github-icon">
                  {/* Icono oficial GitHub */}
                  <svg width="17" height="17" viewBox="0 0 24 24" fill="currentColor">
                    <path d="M12 .297c-6.63 0-12 5.373-12 12 0 5.303 3.438 9.8 8.205
                      11.385.6.113.82-.258.82-.577 0-.285-.01-1.04-.015-2.04-3.338.724
                      -4.042-1.61-4.042-1.61C4.422 18.07 3.633 17.7 3.633 17.7c-1.087
                      -.744.084-.729.084-.729 1.205.084 1.838 1.236 1.838 1.236 1.07
                      1.835 2.809 1.305 3.495.998.108-.776.417-1.305.76-1.605-2.665
                      -.3-5.466-1.332-5.466-5.93 0-1.31.465-2.38 1.235-3.22-.135-.303
                      -.54-1.523.105-3.176 0 0 1.005-.322 3.3 1.23.96-.267 1.98-.399
                      3-.405 1.02.006 2.04.138 3 .405 2.28-1.552 3.285-1.23 3.285-1.23
                      .645 1.653.24 2.873.12 3.176.765.84 1.23 1.91 1.23 3.22 0 4.61
                      -2.805 5.625-5.475 5.92.42.36.81 1.096.81 2.22 0 1.606-.015
                      2.896-.015 3.286 0 .315.21.69.825.57C20.565 22.092 24 17.592 24
                      12.297c0-6.627-5.373-12-12-12"/>
                  </svg>
                </span>
                <span className="footer-social-text">
                  <span className="footer-social-platform">GitHub</span>
                  <span className="footer-social-user">@Ezequie1Sc</span>
                </span>
                <svg className="footer-social-ext" width="13" height="13"
                  viewBox="0 0 24 24" fill="none" stroke="currentColor"
                  strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                  <path d="M7 17L17 7M17 7H7M17 7v10" />
                </svg>
              </a>

            </div>
          </div>

        </div>

        {/* ── Divisor ── */}
        <div className="footer-divider" />

        {/* ── Barra inferior ── */}
        <div className="footer-bottom">
          <p className="footer-copyright">
            © {year} <span>Ezequiel Salazar.</span> Todos los derechos reservados.
          </p>

          <div className="footer-datetime" aria-label="Fecha y hora">
            <svg width="11" height="11" viewBox="0 0 24 24" fill="none"
              stroke="currentColor" strokeWidth="2"
              strokeLinecap="round" strokeLinejoin="round">
              <circle cx="12" cy="12" r="10" />
              <polyline points="12 6 12 12 16 14" />
            </svg>
            {datetime}
          </div>
        </div>

      </div>
    </footer>
  );
};

export default Footer;